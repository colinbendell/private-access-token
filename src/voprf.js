...
import { DataBuffer } from './utils.js';

export class VOPRF {

    constructor(cipherSuite = "P384-SHA384", mode="\x01") {
        this.contextString = VOPRF.createContextString(cipherSuite, mode);
    ...
    */
    computeCompositesFast(k, B, C = [], D = [], hashTofieldConfig) {
        ...
        const seedTranscript = new DataBuffer()
    ...
    *
    * @param {number} k The secret key (scalar form).
    * @param {Point} M The batched form of the blinded tokens.
    * @param {Point} Z The batched form of the signed tokens.
    ...
    */
    generateProof(k, A, B, C, D, r) {
        ...
        const s = this.order + ((r - c * k) % this.order);
    ...
    computeComposites(B, C, D) {
        ...
        for (let i = 0; i < C.length; i++) {
            const Ci = C[i].toRawBytes(true);
            const Di = D[i].toRawBytes(true);
            const compositeTranscript = new DataBuffer()
                .writeInt16(seed.length)
                .writeBytes(seed)
                .writeInt16(i)
                .writeInt16(Ci.length)
    ...
    verifyProof(A, B, C, D, proof) {
        ...
        const challengeTranscript = new DataBuffer()
        ...
        console.log("expectedC", expectedC.toString(16));
